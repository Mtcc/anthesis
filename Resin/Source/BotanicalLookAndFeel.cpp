...
BotanicalLookAndFeel::BotanicalLookAndFeel()
{
    // base colours
    setColour (juce::Slider::thumbColourId,         AC::warmAmber());
...
void BotanicalLookAndFeel::drawRotarySlider (juce::Graphics& g,
    ...
    juce::Slider& /*slider*/)
{
    ...
    // subtle centre highlight
    {
        ...
        juce::ColourGradient cg (AC::warmAmber().withAlpha (0.12f), centre.x, centre.y,
    ...
    if (sliderPos > 0.001f)
    {
        ...
        // amber ghost offset (light catching the vine edge)
        g.setColour (AC::warmAmber().withAlpha (0.3f));
    ...
    // ── 3. indicator: glowing spore/seed ─────────────────────────────────
...
void BotanicalLookAndFeel::drawMacroPetals (juce::Graphics& g,
    juce::Rectangle<float> bounds, float value, float phaseOffset)
{
    ...
    auto centre = bounds.getCentre();
    float radius = juce::jmin (bounds.getWidth(), bounds.getHeight()) * 0.5f;
    float petalR = radius * 1.15f;   // petals extend just beyond knob edge

    // number of petals grows 0→6 with value
    int maxPetals  = 6;
    ...
    auto drawPetal = [&] (float angle, float scale)
    {
        ...
        // petal: small elongated ellipse oriented radially
        juce::Path petal;
        ...
        // colour transitions green→golden with value
    ...
    // partial petal for the last one
...
void BotanicalLookAndFeel::drawLabel (juce::Graphics& g, juce::Label& label)
{
...
void BotanicalLookAndFeel::drawComboBox (juce::Graphics& g, int w, int h,
...
void BotanicalLookAndFeel::positionComboBoxText (juce::ComboBox& box, juce::Label& label)
{
    label.setBounds (6, 1, box.getWidth() - 28, box.getHeight() - 2);
    label.setFont (getComboBoxFont (box));
}

juce::Font BotanicalLookAndFeel::getLabelFont (juce::Label&)
{
    return serifFont;
}

juce::Font BotanicalLookAndFeel::getComboBoxFont (juce::ComboBox&)
{
